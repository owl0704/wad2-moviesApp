    ...
    describe("Filtering", () => {
        ...
        describe("By movie genre and title", () => {
            it("should display movies with the specified genre and title", () => {
              ...
              const searchString = "o";
              const matchingMovies = filterByGenre(movies, selectedGenreId);
              const matchingMovies1 = filterByTitle(matchingMovies, searchString);
              cy.get("select").select(selectedGenreText); 
              cy.get("input").clear().type(searchString) ;
              cy.get(".card").should("have.length", matchingMovies1.length);
              cy.get(".card").each(($card, index) => {
                cy.wrap($card)
                  .find(".card-title")
                  .should("have.text", matchingMovies1[index].title);
              });      
            });
        });
